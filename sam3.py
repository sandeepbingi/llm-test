...
def forecast(channel, type_, future_date):
    ...
    if actual_values:
        ...
        return actual_values, forecast_values, mape, future_dates
    return None, None, None, None

# --- Streamlit UI ---
st.title("📈 Forecast Accuracy Dashboard")

channel_list = ['ny-ios', 'ny-android', 'la-ios', 'la-android']
selected_channel = st.selectbox("Select Channel", channel_list)
selected_type = st.radio("Select Type", ["logins", "payments"])
selected_date = st.date_input("Select Date")
plot_seaborn = st.checkbox("Show Seaborn Heatmap (Residuals)", value=False)
submitted = st.button("Run Forecast")

if submitted:
    actual, predicted, mape, times = forecast(selected_channel, selected_type, pd.to_datetime(selected_date))
    if actual and predicted:
        ...
        # Plotly Line Chart
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=times, y=actual, mode='lines+markers', name='Actual'))
        fig.add_trace(go.Scatter(x=times, y=predicted, mode='lines+markers', name='Forecast'))
        fig.update_layout(title='Actual vs Forecasted Values',
        ...
        st.download_button(
            ...
            file_name=f"{selected_channel}_{selected_type}_{selected_date}_forecast.csv",
            mime="text/csv"
        )

        # Plot Download as HTML
        html_buffer = StringIO()
        fig.write_html(html_buffer, include_plotlyjs='cdn')
        st.download_button(
            label="📥 Download Plot (HTML)",
            data=html_buffer.getvalue(),
            file_name=f"{selected_channel}_{selected_type}_{selected_date}_plot.html",
            mime="text/html"
        )

        # Optional: Seaborn Heatmap + PNG download
        if plot_seaborn:
            ...
            pivot = residual_df.pivot_table(index='hour', values='residuals', aggfunc='mean')
            fig2, ax = plt.subplots(figsize=(8, 4))
            sns.heatmap(pivot.T, cmap="coolwarm", annot=True, ax=ax)
            ax.set_title("Mean Residuals by Hour")
            st.pyplot(fig2)
            ...
            st.download_button(
                ...
                file_name=f"{selected_channel}_{selected_type}_{selected_date}_residuals.png",
                mime="image/png"
            )
